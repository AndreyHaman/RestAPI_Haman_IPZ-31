...
async def get_books(
    ...
    cursor: Optional[datetime] = Query(None, description="Час 'created_at' останньої отриманої книги"),
    status: Optional[BookStatus] = Query(None),
    author: Optional[str] = Query(None),
    sort_by: Optional[str] = Query(None)
):
    service = BookService(db)
    return await service.get_all_books(limit, cursor, status, author, sort_by)
