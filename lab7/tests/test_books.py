...
@pytest.mark.asyncio
async def test_books_creation_granted_with_token(async_client: AsyncClient):
    payload = {"username": "test_user", "password": "superpassword"}
    await async_client.post("/auth/register", json=payload)
    login_res = await async_client.post("/auth/login", json=payload)
    ...
    response = await async_client.post("/books/", json=book_payload, headers=headers)

    assert response.status_code == 201
    assert response.json()["title"] == "Good Book"